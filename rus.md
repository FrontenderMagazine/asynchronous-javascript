...
До `async`-функций уже рукой подать, однако путь к ним был достаточно долгим.
Кажется, ещё недавно мы просто писали [коллбеки][2], и вот уже одна за другой появились [спецификация Promise/A+][9], [генераторы][3], а теперь ещё и `async`-функции.
...
Коллбеки получаются так: если в одну функцию _(т.н. **функцию высшего порядка**)_ в качестве
...
Сам термин был введен [Дэниэлом Фридманом][6] ещё в 1976 году.
...
В средах, пока ещё не поддерживающих промисы нативно, можно использовать полифилы.
...
*Вы спросите: как пользоваться промисами, если большинство библиотек
